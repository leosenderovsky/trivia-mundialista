...
class SoundEffects {
  ...
  public static playReveal() {
    ...
    try {
      const now = ctx.currentTime;
      const notes = [329.63, 440.0, 554.37, 659.25]; // E4, A4, C#5, E5 (A major chord)
      notes.forEach((f, i) => {
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = "sine";
...
export default function App() {
  ...
  const progressPercent = Math.min(
    100,
    Math.round((spentCount / totalQuestions) * 100),
  );
  ...
  return (
    <div
      id="trivia-app-root"
      className="stadium-bg text-white min-h-screen font-sans flex flex-col overflow-x-hidden relative select-none"
    >
        ...
        <section className="relative w-full bg-gradient-to-br from-[#060e2b] via-[#04081c]/95 to-[#02040d] border border-cyan-500/40 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden box-glow-cyan">
          ...
          <div className="flex-1 text-center md:text-left z-10 flex flex-col justify-center">
            <div className="flex items-center justify-center md:justify-start gap-1.5 mb-2 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
              <span className="text-amber-400 text-lg md:text-xl drop-shadow-[0_0_6px_rgba(245,158,11,0.8)]">
                ★
              </span>
              <span className="text-amber-400 text-xl md:text-2xl drop-shadow-[0_0_10px_rgba(245,158,11,0.9)] -translate-y-0.5">
                ★
              </span>
              <span className="text-amber-400 text-lg md:text-xl drop-shadow-[0_0_6px_rgba(245,158,11,0.8)]">
                ★
            ...
            <p className="text-xs md:text-sm font-mono text-slate-400 mt-4 uppercase tracking-widest max-w-md mx-auto md:mx-0">
              Desafío sólo para fanáticos del fútbol
              <br />
              ¡Demostrá tu conocimiento!
            </p>
          </div>
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.className =
                    "w-full h-full object-cover object-top filter contrast-125 saturate-110 drop-shadow-[0_0_15px_rgba(34,211,238,0.3)]";
                }}
              />
            </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
          ...
          <div className="border-lightning-cyan bg-[#071029]/95 p-5 w-full text-center relative overflow-visible rounded-lg">
            <div className="flex items-center justify-between border-b border-cyan-900/60 pb-2.5 mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-anton text-xl tracking-widest text-slate-100 uppercase">
                  GRUPO A
                </span>
              </div>
              <span className="text-[10px] font-mono text-cyan-400 uppercase bg-cyan-950/80 px-2.5 py-0.5 border border-cyan-800/50 rounded">
                LOCAL
              </span>
            </div>
            ...
            <div className="py-2">
              ...
              <p className="text-[10px] text-slate-400 mt-2.5 uppercase tracking-widest font-mono">
                Puntos Acumulados
              </p>
            </div>
            ...
            <div className="flex items-center justify-center gap-4 mt-5 pt-3 border-t border-cyan-900/40 relative">
              ...
              <button
                onClick={() => {
                  SoundEffects.playSelect();
                  setScoreA((prev) => Math.max(0, prev - 100));
                  SoundEffects.playError();
                }}
                className="flex items-center justify-center w-11 h-11 rounded border border-cyan-500/50 hover:bg-cyan-950/70 text-cyan-400 cursor-pointer transition-all active:scale-90 hover:box-glow-cyan"
                title="Restar 100"
              ...
              {showQuickA && (
                <div className="absolute top-[55px] left-1/2 -translate-x-1/2 bg-[#091535] border-2 border-cyan-400 p-3.5 z-30 grid grid-cols-2 gap-2 box-glow-cyan-strong w-52 rounded-lg">
                  ...
                  <button
                    onClick={() => handleScoreChange("A", "add", 100)}
                    className="bg-cyan-950/90 hover:bg-cyan-900 text-cyan-300 py-1.5 text-xs font-mono font-bold border border-cyan-800 rounded transition-colors"
                  >
                    +100
                  </button>
                  <button
                    onClick={() => handleScoreChange("A", "add", 200)}
                    className="bg-cyan-950/90 hover:bg-cyan-900 text-cyan-300 py-1.5 text-xs font-mono font-bold border border-cyan-800 rounded transition-colors"
                  >
                    +200
                  </button>
                  <button
                    onClick={() => handleScoreChange("A", "add", 300)}
                    className="bg-cyan-950/90 hover:bg-cyan-900 text-cyan-300 py-1.5 text-xs font-mono font-bold border border-cyan-800 rounded transition-colors"
                  >
                    +300
                  </button>
                  <button
                    onClick={() => handleScoreChange("A", "add", 400)}
                    className="bg-cyan-950/90 hover:bg-cyan-900 text-cyan-300 py-1.5 text-xs font-mono font-bold border border-cyan-800 rounded transition-colors"
                  >
                    +400
                  </button>
          ...
          <div className="border-lightning-amber bg-[#140f29]/95 p-5 w-full text-center relative overflow-visible rounded-lg">
            <div className="flex items-center justify-between border-b border-amber-900/60 pb-2.5 mb-4">
              ...
              <span className="text-[10px] font-mono text-amber-400 uppercase bg-amber-950/80 px-2.5 py-0.5 border border-amber-800/50 rounded">
                VISITANTE
              </span>
            </div>
            ...
            <div className="flex items-center justify-center gap-4 mt-5 pt-3 border-t border-amber-900/40 relative">
              ...
              <button
                onClick={() => {
                  SoundEffects.playSelect();
                  setScoreB((prev) => Math.max(0, prev - 100));
                  SoundEffects.playError();
                }}
                className="flex items-center justify-center w-11 h-11 rounded border border-amber-500/50 hover:bg-amber-950/70 text-amber-400 cursor-pointer transition-all active:scale-90 hover:box-glow-amber"
                title="Restar 100"
              ...
              {showQuickB && (
                <div className="absolute top-[55px] left-1/2 -translate-x-1/2 bg-[#200d1e] border-2 border-amber-400 p-3.5 z-30 grid grid-cols-2 gap-2 box-glow-amber-strong w-52 rounded-lg">
                  ...
                  <button
                    onClick={() => handleScoreChange("B", "add", 100)}
                    className="bg-amber-950/90 hover:bg-amber-900 text-amber-300 py-1.5 text-xs font-mono font-bold border border-amber-800 rounded transition-colors"
                  >
                    +100
                  </button>
                  <button
                    onClick={() => handleScoreChange("B", "add", 200)}
                    className="bg-amber-950/90 hover:bg-amber-900 text-amber-300 py-1.5 text-xs font-mono font-bold border border-amber-800 rounded transition-colors"
                  >
                    +200
                  </button>
                  <button
                    onClick={() => handleScoreChange("B", "add", 300)}
                    className="bg-amber-950/90 hover:bg-amber-900 text-amber-300 py-1.5 text-xs font-mono font-bold border border-amber-800 rounded transition-colors"
                  >
                    +300
                  </button>
                  <button
                    onClick={() => handleScoreChange("B", "add", 400)}
                    className="bg-amber-950/90 hover:bg-amber-900 text-amber-300 py-1.5 text-xs font-mono font-bold border border-amber-800 rounded transition-colors"
                  >
                    +400
                  </button>
          ...
          <div className="grid grid-board gap-4 md:gap-5 w-full">
            ...
            {categories.map((cat, idx) => (
              ...
              >
                ...
                <h3 className="font-anton text-base md:text-lg lg:text-xl tracking-wide text-white uppercase leading-tight select-none z-10 glow-arg-text">
                  {cat}
                </h3>
              </div>
            ))}
              ...
              categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    {/* Tiny watermark */}
                    {/*{!isSpent && (
                      <span className="absolute top-1.5 right-2 text-[9px] font-mono text-cyan-400/80 font-bold select-none tracking-widest">
                        {val} PTS
                      </span>
                    )}*/}

                    <span
                      className={`font-digital text-3xl md:text-4xl lg:text-5xl font-extrabold ${isSpent ? "text-slate-600" : "text-amber-400 glow-amber group-hover:text-cyan-300"}`}
                    >
                      {val}
                    </span>
                  </div>
      ...
      <footer className="bg-slate-950/80 border-t border-cyan-950/40 py-4 px-6 text-center text-xs font-mono text-slate-500 z-10 flex flex-col sm:flex-row items-center justify-between gap-2">
        <p>
          © 2026 Trivia Mundialista "Los 11 de Echi" | Hecho con amor por "Los
          papis"{" "}
        </p>
        ...
        {showFinalResults && (
          ...
          >
            ...
            <div className="w-full max-w-4xl bg-gradient-to-b from-[#1a1135] to-[#050616] border-lightning-amber p-8 md:p-12 text-center relative z-10 rounded-lg">
              ...
              <h4 className="font-mono text-cyan-400 tracking-widest uppercase text-xs mb-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto my-8">
                ...
                <div className="bg-[#050e26] border border-cyan-500/40 p-6 flex flex-col items-center rounded-lg">
                  <span className="font-mono text-cyan-400 text-xs uppercase tracking-widest">
                    GRUPO A
                  </span>
                  <span className="font-digital text-5xl text-white mt-2 font-bold">
                    {scoreA} PTS
                  </span>
                  {scoreA >= scoreB && (
                ...
                <div className="bg-[#12051c] border border-amber-500/40 p-6 flex flex-col items-center rounded-lg">
                  <span className="font-mono text-amber-400 text-xs uppercase tracking-widest">
                    GRUPO B
                  </span>
                  <span className="font-digital text-5xl text-white mt-2 font-bold">
                    {scoreB} PTS
              ...
              <div className="flex items-center justify-center gap-4 my-6 text-slate-300 max-w-sm mx-auto border-t border-slate-800 pt-6">
                ...
                <div className="text-left text-xs">
                  <p className="font-bold text-slate-100">
                    "Los 11 de Echi" Trivia
                  </p>
                  <p className="text-slate-400 font-mono">
                    ¡Felicitaciones a los mundialistas!
                  </p>
                </div>
              </div>
          ...
          >
            <div className="w-full max-w-2xl bg-[#090f2b] border-2 border-cyan-500 p-6 md:p-8 relative box-glow-cyan">
              ...
              <div className="flex items-center gap-3 border-b border-cyan-900 pb-3 mb-5">
                ...
                <h3 className="font-anton text-3xl tracking-tight uppercase text-white">
                  REGLAS DEL JUEGO (JEOPARDY)
                </h3>
              </div>
              ...
              <div className="text-slate-300 text-sm md:text-base leading-relaxed flex flex-col gap-4 select-text">
                <p>
                  Bienvenido a la{" "}
                  <strong>Trivia Mundialista: Los 11 de Echi</strong>. Jugá y
                  competí como un campeón siguiendo estas pautas de juego show
                  televisivo:
                </p>
                <ul className="list-decimal list-inside flex flex-col gap-2.5 bg-slate-950/60 p-4 border border-cyan-900/50">
                  <li>
                    Formen dos grupos: el <strong>Grupo A (Local)</strong> y el{" "}
                    <strong>Grupo B (Visitante)</strong>.
                  </li>
                  <li>
                    Elijan un moderador/facilitador (el Director Técnico de la
                    noche).
                  </li>
                  <li>
                    Por turnos, cada grupo elige una celda disponible en la
                    grilla indicando la categoría y el puntaje (ej.{" "}
                    <em>"Estadios por 200 puntos"</em>).
                  </li>
                  <li>
                    El moderador hace clic en la celda correspondiente para
                    abrir el panel y lee la pregunta gigante.
                  </li>
                  <li>
                    Ambos grupos pueden competir por responder en un tiempo
                    límite o usar pulsadores caseros.
                  </li>
                  <li>
                    Al revelarse la respuesta, se asigna el puntaje
                    correspondiente al grupo ganador con los controles de sumas
                    y restas del modal.
                  </li>
                  <li>
                    Al finalizar todas las preguntas, presionen{" "}
                    <strong>"Finalizar Juego"</strong> para coronar al campeón
                    de la Copa Trivia Mundialista.
                  </li>
                </ul>
              </div>
        ...
        {showResetConfirm && (
          ...
          >
            <div className="w-full max-w-md bg-[#190d16] border-2 border-red-500 p-6 text-center shadow-[0_0_20px_rgba(239,68,68,0.2)]">
              <h3 className="font-anton text-2xl tracking-tight text-white uppercase mb-2">
                ¿Reiniciar partido?
              </h3>
              <p className="text-slate-300 text-sm mb-6 leading-relaxed select-text">
                Esto borrará todos los puntos de los grupos y restablecerá todas
                las preguntas del tablero como no usadas. Esta acción es
                irreversible.
              </p>
